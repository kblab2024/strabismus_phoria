...
# ===== 使用者設定 =====
IMAGE_PATH = r"C:\Users\Tristan\Downloads\picswithlens\S__159744057_0.jpg"  # 請改成你的圖片路徑
# ====================

# =====開發工具：美化console結果=====
console = Console()
#為使console結果美觀，不要讓所有提示訊息全部混在一起，所以有以下規則
#extraction: 藍色
#save: 黃色
#開發中印出東西: 紫色
#語法：將本來的print()換成console.print('', style='color')
# ==================================
...
def process_frame(frame, fm):
    ...
    value_for_true_radius_calculation = ()
    for nummer, (iris_idx, color) in enumerate(((LEFT_IRIS, (0,255,0)), (RIGHT_IRIS, (0,255,0)))):
        #note:　照片中的人跟我們呈鏡像反射，所以先出來的是他的左眼，我們的右邊。這點跟ct或MRI一樣
        ...
        if 'IMAGE_PATH' in globals() and IMAGE_PATH and detected_ellipse is not None:
            ...
            if iris_idx == LEFT_IRIS:
                value_for_true_radius_calculation = detected_ellipse
            ...
            ellipse_out = cv2.bitwise_and(circle_roi, circle_roi, mask=mask)
            
            cv2.imwrite(str(outpath/f"largest_circle_{nummer+1}.png"), ellipse_out)
            ...
            blue_bkg_iris = np.full_like(iris_roi, (255,0,0))
            inv_mask_iris = cv2.bitwise_not(mask)
            blue_part_iris = cv2.bitwise_and(blue_bkg_iris, blue_bkg_iris, mask=inv_mask_iris)
            
            # 5. Apply mask to keep only circular region
            iris_circle = cv2.bitwise_and(iris_roi, iris_roi, mask=mask)
            
            output_iris = cv2.add(iris_circle, blue_part_iris)
            # Step.1 先刪全藍 row（行）
            is_blue_row = np.all(output_iris == [255, 0, 0], axis=(1,2))
            tmp = output_iris[~is_blue_row, :, :]
            
            # Step.2 再刪全藍 column（列）
            is_blue_col = np.all(tmp == [255, 0, 0], axis=(0,2))
            result = tmp[:, ~is_blue_col, :]

            
            cv2.imwrite(str(outpath/f"{nummer+1}.png"), result)
        """
    ...
    return vis, value_for_true_radius_calculation

def detect_bright_spots_in_output(transverse_pixel):
    """
    ...
    magnifying_ratio = 24/transverse_pixel
    ...
    for file in outpicpath.rglob('*.png'):
        ...
        if 'largest' in file.name:
            continue   #大圓的不用找
            
        console.print(f'Extracting bright spot from {file.name}',style = 'green')
        ...
        #開始算實際偏移量
        center_pixel = (img.shape[0]-1)/2
        arr = np.array(foundspot)           # arr.shape = (N, 2)
        mean_xy = np.mean(arr, axis=0)       # mean_xy 是 [平均x, 平均y]
        delta_y = (mean_xy[0] - center_pixel)*magnifying_ratio*22
        delta_x = (mean_xy[1] - center_pixel)*magnifying_ratio*22
        
        print(file.name, delta_x, delta_y)
        ...
        plt.close(fig)
        console.print(f"Saved: {save_path.name}", style = 'yellow')

def pixel_of_the_radius_calculation(val):
    #value的格式：((ecx_roi, ecy_roi), (eax, eby), eang)
    (ecx_roi, ecy_roi), (eax, eby), eang = val
    eang = math.radians(eang)
    pixel = math.sqrt(1/(((math.cos(eang)/eax)**2)+((math.sin(eang)/eby)**2))) ###需要檢查這樣算到底是對的嗎？長軸短軸有無弄反
    return pixel
...
if __name__ == '__main__':
    ...
    with mp_face_mesh.FaceMesh(
        ...
        out, value_for_true_radius_calculation = process_frame(img, fm)
        
    transverse_pixel = pixel_of_the_radius_calculation(value_for_true_radius_calculation)
    
    cv2.imshow('facemesh_iris_pupil_single', out)
    # now detect bright spots in the output image
    detect_bright_spots_in_output(transverse_pixel)
    output_path = Path(IMAGE_PATH).parent / 'output' / f'{Path(IMAGE_PATH).stem}_result.png'
    output_path.parent.mkdir(parents=True, exist_ok=True)
    cv2.imwrite(str(output_path), out)
    console.print(f"Saved result to: {output_path}", style= 'yellow')
    cv2.waitKey(0)
